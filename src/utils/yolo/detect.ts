...
const preprocess = (source: any, modelWidth: number, modelHeight: number) => {
  ...
  const input = tf.tidy(() => {
    ...
    const imgPadded = img.pad([
      ...
      [0, 0],
    ]) as tf.Tensor3D; // pad image to square
...
export const detect = async (source, model, canvasRef, callback = () => {}) => {
  ...
  const boxes = tf.tidy(() => {
    ...
    return tf
      ...
      .squeeze();
  }) as tf.Tensor2D; // process boxes [y1, x1, y2, x2]
  ...
  // Otherwise, add newly detected classes to the ignored array
  classes_data.forEach((cls) => {
    if (!ignoredClasses.includes(cls as never) && scores_data[0] > certain) {
      ignoredClasses.push(cls as never);
    }
  });
 ...
 * @param {HTMLCanvasElement} canvasRef canvas reference
 */
export const detectVideo = (vidSource, model, canvasRef, setSeen) => {
  ignoredClasses = []; // reset ignored classes
  const FPS = 30; // Desired FPS (e.g., 15 fps)
  const frameInterval = 1000 / FPS; // Interval in ms between frames
  let lastFrameTime = 0;
